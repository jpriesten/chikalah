...
exports.add = async (req, res) => {
    ...
    else if (!phone.get("phone"))
        return response.errorResponse(res, 404, "Missing required field 'phone'", 40004);
    ...
    try {
        ...
        if (error?.code === 11000)
            response.errorResponse(res, 409, "Phone number already exists", error.code);
...
exports.findAll = async (req, res) => {
    ...
    try {
        let tels = {};
        if (Object.keys(req.query).length === 0) tels = await Phone.find();
        else {
            let params = {};
        ...
        response.successResponse(res, 200, [
            {tels, count: Object.keys(tels).length},
        ]);
    } catch (error) {
        response.errorResponse(res, 500, error.message, error.code);
    }
